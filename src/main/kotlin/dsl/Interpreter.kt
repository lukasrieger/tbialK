 ...
 */
fun interface Interpreter<C, A> {

    context(C, HandlerContext)
    @Suppress("SUBTYPING_BETWEEN_CONTEXT_RECEIVERS")
    suspend fun Action<A>.execute(): A
}

fun <A> defaultClientInterpreter() = object : Interpreter<GameSessionContext, A> {
    context(GameSessionContext, HandlerContext)
...
fun <A> defaultServerInterpreter() = object : Interpreter<ServerGameSessionContext, A> {
    context(ServerGameSessionContext, HandlerContext)
